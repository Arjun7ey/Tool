...
const DocumentTable = () => {
    ...
    const handleUpload = () => {
        ...
        if (!file) {
            setSnackbarMessage('Please select a document to upload');
            setSnackbarOpen(true);
            return;
        }
            ...
            html: (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                    ...
                    >
                        ...
                        <Typography>Click to view document</Typography>
                    </a>
                    
                    {(userRole === 'superadmin' || userRole === 'departmentadmin') && (
                       <div style={{ marginTop: '20px', textAlign: 'center' }}>
                       <h4 style={{ marginBottom: '10px' }}>Rate this Document:</h4>
    ...
    const handleDelete = (selectedTasks) => {
        ...
        MySwal.fire({
            title: 'Are you sure?',
            text: `You are about to delete ${selectedTasks.length > 1 ? 'these documents' : `"${selectedTasks[0].title}"`}.`,
            icon: 'warning',
    ...
    const handleDivisionFilter = (divisionName) => {
        const newDivision = divisionName === 'all' ? '' : divisionName;
        setFilterDivision(newDivision);
        filterTasks(tasks, statusFilter, newDivision);
    };
                ...
                <Drawer className="UploadDrawer" anchor="right" open={openUploadDrawer} onClose={handleCloseUploadDrawer}>
                    <Box sx={{ position: 'relative', height: '100%', width: 300, padding: 3 }}>
                        ...
                        <div className="DropzoneArea" {...getRootProps()}>
                            ...
                            {isDragActive ? (
                                <Typography>Drop the document file here...</Typography>
                            ) : (
                                <Typography>Drag and drop a document file here, or click to select</Typography>
                            )}
                        </div>
                        {file && (
